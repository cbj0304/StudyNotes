  ...
  * c++代码实现
  ...
  ```c++
    ...
    void BFS(map<char, vector<char>> & graph) {
        ...
        queue<char> _queue;
        auto it = graph.begin();
        char cur_node = it->first;
        _queue.push(cur_node);

        while (!_queue.empty()) {
            cur_node = _queue.front();
            ...
            for(int i=0; i<graph[cur_node].size();i++) {
                // 未访问过就加入队列.queue里边的一定都是未访问的
                if (visited.find(graph[cur_node][i]) == visited.end()) {
                    _queue.push(graph[cur_node][i]);
                }
            }
        }
    }
...
* 有权图计算最小路径
  ...
  基础知识：
  python优先级队列：
  和c++中stl-priority_queue类似，通过堆实现优先级队列，引入模块 import heapq
  ...
  * c++代码实现
  ...
  ```c++
    ...
    // 计算有权图的最短路径
    void BFS_minPath(map<char, map<char, int>> & graph) {
        if (graph.size() == 0) {
            return;
        }

        int size = graph.size();
        map<char, char> _parent;  // 记录节点的父节点，用于回溯
        map<char, int> _minPath;         // 记录到达该顶点的最短路径
        for(auto it=graph.begin(); it != graph.end(); it++) {
            _minPath.insert(make_pair(it->first, MAX_PATH));
            _parent.insert(make_pair(it->first, '0'));
        }

        auto it = graph.begin();
        priority_queue<pair<char, int>, vector<pair<char, int>>, my_greater> _queue;
        _queue.push(make_pair(it->first, 0));
        _parent[it->first] = '0';
        _minPath[it->first] = 0;
        ...
        while (!_queue.empty()) {
            ...
            map<char, int> child_nodes = graph[node];
            for(auto it=child_nodes.begin(); it != child_nodes.end(); it++) {
                char child_node = it->first;
                int child_dist = it->second;
                if (visited.find(child_node) != visited.end()) {
                    continue;
                }

                if (dist + child_dist < _minPath[child_node]) {
                    _minPath[child_node] = dist + child_dist;
                    _parent[child_node] = node;
                }
                _queue.push(make_pair(child_node, _minPath[child_node]));
            }
        }

        cout << "parent..." << endl;
        for(auto it = _parent.begin(); it != _parent.end(); it++) {
            cout << it->first << ", " << it->second << endl;
        }
        cout << endl << "minPath..." << endl;
        for(auto it = _minPath.begin(); it != _minPath.end(); it++) {
            cout << it->first << ", " << it->second << endl;
        }
    }
